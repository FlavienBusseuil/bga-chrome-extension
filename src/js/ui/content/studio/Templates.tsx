...
const Templates = (props: TemplatesProps) => {
	...
	useEffect(
		() =>
			setJsonTemplates(
				JSON.stringify(templates, ["name", "text", "game"], 2),
			),
		[templates],
	);
	...
	const addTemplate = () => {
		setTemplates(
			config.addTemplate({
				name: "my sentence name",
				text: "my sentence text",
				game: gameName,
			}),
		);
	};
	...
	const getOptions = () => {
		...
		return getTemplates().map((t, index) => {
			return (
				<option key={`o_${index}`} value={t.text}>
					{t.name}
				</option>
			);
		});
	...
	const getSentences = () => {
		...
		return getTemplates().map((t, index) => {
			const updateTemplate = (
				...
				setTemplates(
					config.updateTemplate(t.name, t.game, { game, name, text }),
				);
			};
			...
			return (
				<div className="bgext_studio_sentence" key={`s_${index}`}>
					...
					<div className="bgext_studio_sentence_game">
						<input
							type="checkbox"
							checked={t.game === "all" || t.game === undefined}
							id={checkId}
							onChange={(evt) =>
								updateTemplate(
									t.name,
	...
	if (configMode) {
		...
		return (
			<div className="bgext_studio_row">
				<h3
					className="pagesection__title"
					style={{ paddingLeft: "2em" }}
				>
					Template messages
				</h3>
				<div className="bgext_studio_big_container">
					{!plainMode && existsTemplates && (
						<div className="bgext_studio_sentences">
							<div className="bgext_studio_sentence">
								<div className="bgext_studio_sentence_name">
									<div className="bgext_studio_sentence_col_title">
										Name
									</div>
								</div>
								<div className="bgext_studio_sentence_text">
				...
				<div className="bgext_studio_sentence_hor_buts">
					{jsonError && (
						<span style={{ color: "red", padding: "16px 4px" }}>
							JSON Error!
						</span>
					)}
					{!plainMode && (
					...
					>
						{plainMode ? "normal display" : "json display"}
					</a>
					<a
						className="bgabutton bgabutton_blue"
	...
	return (
		<div className="bgext_studio_container">
			<div className="bgext_studio_select_container">
				{existsTemplates && (
					<select id="templates_input">{getOptions()}</select>
				)}
				{!existsTemplates && (
					<div className="bgext_studio_no_template">
						No templates found
					</div>
				)}
			</div>
			{existsTemplates && (
				<a className="bgabutton bgabutton_blue" onClick={useTemplate}>
					insert this template
				</a>
			)}
			<a className="bgabutton bgabutton_blue" onClick={toggleConfig}>
				config
			</a>
		</div>
	);
